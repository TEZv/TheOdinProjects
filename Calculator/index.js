...
function onOpClicked(op) {
  if (firstNum === "" && !userInput && result === "") {
    //first thing pressed is operator
    firstNum = 0;
    operator = op;
    handleDisplay();
  } else if (userInput && firstNum === "") {
    // num pressed then operator
    firstNum = +userInput;
    userInput = "";
    result = "";
    operator = op;
    ...
    //num pressed then operator then num then operator
    secondNum = +userInput;
    ...
    operator = op;
    handleDisplay();
  } else if (result !== "" && !userInput && firstNum === "") {
    // already calculated and operator is pressed
    firstNum = result;
    result = "";
    operator = op;
    handleDisplay();
...
function handleBackspace() {
  if (firstNum !== "" && operator && userInput) {
    //if firstNum and operator are saved delete userInput
    userInput = userInput.slice(0, -1);
    handleDisplay();
  } else if (firstNum !== "" && operator) {
    //once userInput has been deleted delete operator
    operator = "";
    handleDisplay();
  } else if (firstNum !== "") {
    // once operator has been deleted delete firstNum
    firstNumStr = firstNum.toString().slice(0, -1);
    if (firstNumStr) {
      //check to see if firstNum still exists
