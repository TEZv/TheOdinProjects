function checkFirstName(element) {
  if (element.validity.valueMissing) {
    ...
    element.setCustomValidity(
      "Should have 2 through 15 letters:) Too short or too long, try again."
    );
    element.reportValidity();
    return false;
...
function checkPassword(element) {
  if (element.value == "") {
    ...
    element.setCustomValidity(
      "Must contain 8 characters one uppercase, lowercase, and one number."
    );
    element.reportValidity();
    return false;
  } else {
...
inputElementArray.forEach((i) =>
  i.addEventListener("input", (e) => validityCheckArray[e.target.id](e.target))
);
